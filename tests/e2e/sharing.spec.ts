...
test('two browsers receive person, activity creation, edits, assignments and deletion through SSE', async ({ browser }) => {
  ...
  try {
    ...
    await expect(second.getByRole('button', { name: 'Select Sync Football for scheduling' })).toBeVisible();
    await first.getByRole('button', { name: 'Select Sync Football for scheduling' }).click();
    await first.getByRole('button', { name: /^Schedule Sync Football on/ }).first().click();
    ...
    const editor = first.getByRole('region', { name: 'Edit scheduled activity' });
    ...
    await expect(second.getByRole('region', { name: 'Edit scheduled activity' }).getByRole('checkbox', { name: 'Sync Alex' })).toBeChecked();
...
test('dirty draft survives remote edit, stale save returns 409, explicit review allows retry', async ({ browser }) => {
  const a = await browser.newContext(), b = await browser.newContext();
  const first = await a.newPage(), second = await b.newPage();
  try {
    ...
    const editorA = first.getByRole('region', { name: 'Edit scheduled activity' });
    const editorB = second.getByRole('region', { name: 'Edit scheduled activity' });
    ...
    const conflict = first.waitForResponse(r => r.request().method() === 'PATCH' && r.status() === 409);
    await editorA.getByRole('button', { name: 'Save', exact: true }).click(); await conflict;
    await expect(editorA.getByRole('textbox', { name: 'Title', exact: true })).toHaveValue('My unsaved draft');
...
test('failed optimistic save rolls back the card and keeps the draft; offline edits are refused', async ({ page, context }) => {
  ...
  const editor = page.getByRole('region', { name: 'Edit scheduled activity' });
